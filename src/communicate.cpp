 ...
 *  message tag and mpi_communicators are assigned by default if user doesnt
 *assign them
...
CommPoint2Point<Type>::CommPoint2Point( void *buff, uint size, uint *tg, uint snd, uint rcv, uint type,
                                        MPI_Comm Comm ) /*!< If communicator is
                                                           ...
                                                           default*/
{
    ...
    if ( tg == nullptr )
    {
        Msg.tag = 0; /*!< default tag for message send and recieve is "0",
                        MPI_ANY_TAG is onlu OK for recieve commnad so it can not be
                        used here*/
    }
    else
    {
...
template <class Type>
CommPoint2Point<Type>::CommPoint2Point( void *buff, uint size, uint snd, uint rcv ) /*!< another constructor for
                                                                                       Communicator class*/
{
    Com.mpiCom = MPI_COMM_WORLD;
...
template <class Type>
CommPoint2Point<Type>::CommPoint2Point( void *buff, uint size ) /*!< another constructor for Communicator class,
                                                                   deferes assigning sender and reciever*/
{
    Com.mpiCom = MPI_COMM_WORLD;
...
// I wanted to template this fucntion too but due to the presence of nullptr_t
// is my MPI_Templitization, this did fail
//  since + sign for nullptr's is undefined, we can come back to this after we
// decide on what to do on nullptr_t
//  as I reserved that for MPI_TYPE_BYTE which I need for Morton encoding

template <class Type>
void CommPoint2Point<Type>::getOffset( uint myvalue, uint *offset ) /*!<gets the number of
                                                                       elements before the
                                                                       current processor, it is
                                                                         needed for globally
...
template <class Type>
CommCollective<Type>::CommCollective( void *buff, uint size, uint root ) /*!< another constructor for
                                                                            Communicator class*/
{
    Com.mpiCom = MPI_COMM_WORLD;
...
template <class Type>
CommCollective<Type>::CommCollective( void *buff, uint size ) /*!< another constructor for Communicator class, this one sets
                                                                 the root as comSize-1 for hdf5*/
{
    Com.mpiCom = MPI_COMM_WORLD;
